...
{input_text}"""
    ...
    def _generate_html(self, original: str, translation: str, english: str, source_lang: str, target_lang: str) -> str:
        ...
        # ✅ FIX: escape nội dung trước khi chèn vào HTML — trước đây chèn
        # thẳng (raw) nên nếu văn bản gốc/bản dịch chứa <, >, & sẽ làm hỏng
        # cấu trúc HTML xuất ra (thẻ bị cắt ngang, layout vỡ).
        safe_original = _html_escape(original or "")
        safe_translation = _html_escape(translation or "")
        safe_english = _html_escape(english or "") if english else ""
...
<body>
 ...
 <div class="section original">
  ...
  <div class="content">{safe_original}</div>
 </div>

 <div class="section translation">
  <div class="label">Translation ({target_lang})</div>
  <div class="content">{safe_translation}</div>
 </div>
"""
 ...
 <div class="section english">
  ...
  <div class="content">{safe_english}</div>
 </div>
"""
